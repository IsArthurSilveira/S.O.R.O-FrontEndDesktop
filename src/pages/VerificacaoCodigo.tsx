  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_URL}/auth/reset-password`, {
        ...
        body: JSON.stringify({ email, token: code, password: newPassword }),
      });

      const data = await response.json();

      if (response.ok) {
        setMessage({ type: 'success', text: 'Senha redefinida com sucesso! Você pode fazer login agora.' });
      } else {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background">
      ...
      >
        ...
        <h2 className="text-2xl font-bold text-center text-foreground">Redefinir Senha</h2>
        <p className="text-sm text-center text-muted-foreground">Insira o código enviado por e-mail e sua nova senha.</p>
